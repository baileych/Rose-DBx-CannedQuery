...
sub new { 
  state $i = 1;
  bless { serial => $i++,
	  sth => { Active => 1, Database => { Active => 1} } }, shift;
}
...
ok($cache->add_query_to_cache($dummy0, $keyargs),
  'Add item to cache');
...
is($cache->get_query_from_cache({ map { tr/A-Za-z/a-zA-Z/; s/ /  /; $_ }
			      %$keyargs }),
   $dummy0,
...
ok(! $cache->get_query_from_cache($keyargs),
...
ok($cache->add_query_to_cache($dummy1, $keyargs),
  'Replace item in cache');
is($cache->get_query_from_cache($keyargs),
   $dummy1,
   'Retrieve new item from cache');


ok($cache->remove_query_from_cache($keyargs),
   'Remove object');
ok(! $cache->get_query_from_cache($keyargs),
...
ok(! $cache->get_query_from_cache($keyargs2),
   'Cached object is gone');
